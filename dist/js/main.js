...
if($('#typing-area').length>0){
	inputField.addEventListener('keydown', e => {
	  ...
	  function inputFieldClass() {
		if (e.key >= 'a' && e.key <= 'z' || (e.key === `'` || e.key === ',' || e.key === '.' || e.key === ';' || e.key === 'ş' || e.key === 'ç' || e.key === 'ı'|| e.key === 'ğ' || e.key === 'ö' || e.key === 'ı' || e.key === 'ü')) {
...
function showResult() {
  ...
  let wpm = Math.floor(words / minute);
	document.querySelector('#right-wing').innerHTML = `Dakika Başı Kelime: ${wpm} / Toplam Harf: ${acc}`;
