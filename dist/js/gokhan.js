...
$.getJSON('./dist/haberler.json', function(response) {    
	...
	for (let i = 0; i < window.haberlerArr.length &&  i < window.sayfadakiHaber; i++) {
	  ...
	  html = html + '<div> <div class="row"><div class="col-auto"><span class="gokhanavatar" style="background-image: url(' + window.haberlerArr[i]['resim'] +')"></span></div><div class="col"><div class="text-truncate p-1">' + window.haberlerArr[i]['baslik'] + '</div><div class="text-muted p-1">' + window.haberlerArr[i]['metin'].substring(0,200) +'</p></div><a href="' + window.haberlerArr[i]['id'] +  '" class="btn btn-ghost-info float-end"> Devamını okuma</a></div></div></div>';
...
function paginate(array, page_size, page_number) {
	...
	for (let i = 0; i < sliced.length; i++) {
	  ...
	  html = html + '<div> <div class="row"><div class="col-auto"><span class="gokhanavatar" style="background-image: url(' + sliced[i]['resim'] +')"></span></div><div class="col"><div class="text-truncate p-1">' + sliced[i]['baslik'] + '</div><div class="text-muted p-1">' + sliced[i]['aciklama'].substring(0,200) +'</p><a href="/oku?id=' + sliced[i]['id'] +  '" class="btn btn-ghost-info float-end"> Tümünü Oku</a></div><hr/></div></div>';
